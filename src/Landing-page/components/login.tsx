    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{
        ...
        <input
          ...
          style={{ flex: 1, border: "none", outline: "none", padding: "11px 14px", fontSize: 15, color: "#111827", background: "transparent", minWidth: 0 }}
        />
        {isPass && (
...
const Select = ({ label, value, onChange, options, placeholder }: {
  ...
  <div style={{ marginBottom: 14 }}>
    ...
    <select value={value} onChange={(e) => onChange(e.target.value)}
      style={{ width: "100%", border: "1.5px solid #E5E7EB", borderRadius: 12, padding: "11px 14px", fontSize: 15, color: value ? "#111827" : "#9CA3AF", outline: "none", background: "#fff", cursor: "pointer", boxSizing: "border-box" as const }}>
...
const Textarea = ({ label, placeholder, value, onChange }: {
  ...
  <div style={{ marginBottom: 14 }}>
    ...
    <textarea value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} rows={3}
      style={{ width: "100%", border: "1.5px solid #E5E7EB", borderRadius: 12, padding: "11px 14px", fontSize: 15, color: "#111827", outline: "none", resize: "none", fontFamily: "inherit", boxSizing: "border-box" as const }} />
  </div>
);
    ...
    {Array.from({ length: total }).map((_, i) => (
      <React.Fragment key={i}>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
          ...
          <span style={{ fontSize: 9, color: i === current ? "#15803D" : "#9CA3AF", fontWeight: i === current ? 700 : 400, whiteSpace: "nowrap" as const }}>
            {labels[i]}
          </span>
        </div>
        {i < total - 1 && (
          <div style={{ flex: 1, height: 2, background: i < current ? "#15803D" : "#E5E7EB", margin: "0 4px", marginBottom: 14, transition: "background 0.3s" }} />
        )}
      </React.Fragment>
    ))}
  </div>
...
const MobileHeader = ({ page, role }: { page: AuthPage; role: Role }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ width: 48, height: 48, borderRadius: "50%", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0, boxShadow: "0 4px 16px rgba(0,0,0,0.15)" }}>🔧</div>
      <div>
        <div style={{ fontSize: 13, fontWeight: 800, color: "#F97316", letterSpacing: "0.5px" }}>SmartServe</div>
        <div style={{ fontSize: 18, fontWeight: 800, color: "#fff", lineHeight: 1.2 }}>
          {page === "login" ? "Welcome Back!" : isProvider ? "Grow Your Business!" : "Get Started!"}
        </div>
        <div style={{ fontSize: 12, color: "rgba(255,255,255,0.65)", marginTop: 2 }}>
          {page === "login" ? "Sign in to your account" : isProvider ? "Join 50K+ professionals" : "Professional services at your fingertips"}
        </div>
      </div>
    </div>
  );
...
const LoginForm = ({
  ...
  return (
    <div style={{ width: "100%", maxWidth: 400 }}>
      <h1 style={{ fontSize: 28, fontWeight: 800, color: "#111827", margin: "0 0 4px", letterSpacing: "-0.5px" }}>welcome</h1>
      <p style={{ fontSize: 14, color: "#6B7280", marginBottom: 20 }}>Please enter your details to access your dashboard.</p>

      {/* Mode switcher */}
      <div style={{ background: "#F3F4F6", borderRadius: 12, padding: 4, display: "flex", marginBottom: 20, gap: 2 }}>
        {([["auto","🔍 Auto-detect"],["manual","🔘 Choose role"]] as const).map(([m, label]) => (
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ position: "relative" }}>
            <input
              ...
              style={{ width: "100%", border: `1.5px solid ${detectedRole ? "#15803D" : "#E5E7EB"}`, borderRadius: 12, padding: "11px 48px 11px 14px", fontSize: 15, outline: "none", boxSizing: "border-box" as const, transition: "border-color 0.2s" }}
            />
          ...
          {mode === "auto" && detecting && (
            <div style={{ marginTop: 5, fontSize: 12, color: "#9CA3AF" }}>Looking up account...</div>
          )}
        </div>
        ...
        {error && (
          <div style={{ background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 10, padding: "10px 14px", marginBottom: 12, fontSize: 13, color: "#DC2626", display: "flex", gap: 8, alignItems: "flex-start" }}>
            <span>⚠️</span><span>{error}</span>
          </div>
        )}

        <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 12, color: "#3B82F6" }}>
          <strong>Demo accounts:</strong><br />
          📧 user@smartserve.com / user1234<br />
          📧 provider@smartserve.com / password
        </div>
        ...
        <button type="submit" disabled={loading} style={{
          ...
          cursor: loading ? "not-allowed" : "pointer", transition: "background 0.2s", marginBottom: 16,
      ...
      <div style={{ marginTop: 16, padding: "12px 16px", background: "#F9FAFB", borderRadius: 12, border: "1px solid #F3F4F6" }}>
        ...
        <div style={{ fontSize: 12, color: "#6B7280", lineHeight: 1.6 }}>
          ...
          <strong>Manual:</strong> Sends <code>roleHint</code> to <code>/auth/login</code>.
        </div>
      </div>
    </div>
  );
...
const RoleSelector = ({ onSelect }: { onSelect: (r: Role) => void }) => (
  <div style={{ width: "100%", maxWidth: 420 }}>
    <h1 style={{ fontSize: 26, fontWeight: 800, color: "#111827", margin: "0 0 4px", letterSpacing: "-0.5px" }}>Create your account</h1>
    <p style={{ fontSize: 14, color: "#6B7280", marginBottom: 28 }}>Choose how you want to use SmartServe</p>
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <button onClick={() => onSelect("user")} style={{ background: "#fff", border: "2px solid #E5E7EB", borderRadius: 16, padding: "20px", cursor: "pointer", textAlign: "left", transition: "all 0.2s", width: "100%" }}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: 16, color: "#111827", marginBottom: 3 }}>I'm a Customer</div>
            <div style={{ fontSize: 13, color: "#6B7280" }}>Book professional home services near you.</div>
          </div>
      ...
      <button onClick={() => onSelect("provider")} style={{ background: "#fff", border: "2px solid #E5E7EB", borderRadius: 16, padding: "20px", cursor: "pointer", textAlign: "left", transition: "all 0.2s", width: "100%" }}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: 16, color: "#111827", marginBottom: 3 }}>I'm a Service Provider</div>
            <div style={{ fontSize: 13, color: "#6B7280" }}>List your services, manage appointments & grow.</div>
          </div>
          <span style={{ fontSize: 18, color: "#9CA3AF", flexShrink: 0 }}>→</span>
        </div>
        <div style={{ display: "flex", gap: 6, marginTop: 12, flexWrap: "wrap" as const }}>
...
const UserSignupForm = ({
  ...
  return (
    <div style={{ width: "100%", maxWidth: 400 }}>
      ...
      <h1 style={{ fontSize: 24, fontWeight: 800, color: "#111827", margin: "8px 0 4px", letterSpacing: "-0.5px" }}>Create your account</h1>
      <p style={{ fontSize: 14, color: "#6B7280", marginBottom: 20 }}>Book professional services in minutes</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? "0" : "0 14px" }}>
      ...
      <label style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 20, cursor: "pointer" }}>
        <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} style={{ marginTop: 2, accentColor: "#15803D", width: 16, height: 16, cursor: "pointer", flexShrink: 0 }} />
      ...
      <button onClick={handleSubmit} disabled={loading}
        style={{ width: "100%", background: loading ? "#86EFAC" : "#15803D", color: "#fff", fontWeight: 700, fontSize: 15, padding: "14px 0", borderRadius: 12, border: "none", cursor: loading ? "not-allowed" : "pointer", marginBottom: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
...
const PROVIDER_STEPS = ["Account","Service","Work","Verify"];
...
const ProviderSignupForm = ({
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" as const }}>
              {["Mon","Tue","Wed","Thu","Fri","Sat","Sun"].map((d) => (
                <button key={d} style={{ padding: "7px 12px", borderRadius: 99, fontSize: 12, fontWeight: 600, background: ["Mon","Tue","Wed","Thu","Fri"].includes(d) ? "#15803D" : "#F3F4F6", color: ["Mon","Tue","Wed","Thu","Fri"].includes(d) ? "#fff" : "#6B7280", border: "none", cursor: "pointer" }}>{d}</button>
              ))}
            </div>
          </div>
          <div style={{ marginBottom: 14 }}>
            ...
            <input value={form.skills} onChange={(e) => set("skills")(e.target.value)}
              ...
              style={{ width: "100%", border: "1.5px solid #E5E7EB", borderRadius: 12, padding: "11px 14px", fontSize: 14, outline: "none", boxSizing: "border-box" as const }} />
            <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 6, marginTop: 8 }}>
              {form.skills.split(",").map((s) => s.trim()).filter(Boolean).map((s) => (
                <span key={s} style={{ padding: "4px 10px", background: "#F0FDF4", color: "#15803D", border: "1px solid #BBF7D0", borderRadius: 99, fontSize: 12, fontWeight: 500 }}>{s}</span>
              ))}
            </div>
          </div>
          <div style={{ background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 12, padding: "12px 16px", fontSize: 13, color: "#15803D" }}>
            💡 Add portfolio photos and certifications after creating your account
          </div>
        </>
      );
      case 3: return (
        <>
          ...
          <div style={{ background: "#F9FAFB", borderRadius: 14, padding: 14, marginBottom: 14 }}>
            ...
            {[
              ...
              <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 13, padding: "4px 0", borderBottom: "1px solid #F3F4F6", gap: 8 }}>
                <span style={{ color: "#9CA3AF", flexShrink: 0 }}>{k}</span>
                <span style={{ color: "#374151", fontWeight: 500, textAlign: "right", wordBreak: "break-word" as const }}>{v}</span>
              </div>
            ))}
          </div>

          {apiError && (
            <div style={{ background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 10, padding: "10px 14px", marginBottom: 12, fontSize: 13, color: "#DC2626", display: "flex", gap: 8 }}>
              <span>⚠️</span><span>{apiError}</span>
            </div>
          )}

          <label style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 8, cursor: "pointer" }}>
            <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} style={{ marginTop: 2, accentColor: "#15803D", width: 16, height: 16, flexShrink: 0 }} />
            <span style={{ fontSize: 13, color: "#6B7280", lineHeight: 1.5 }}>
              I agree to the <a href="#" style={{ color: "#15803D", fontWeight: 600 }}>Terms</a>, <a href="#" style={{ color: "#15803D", fontWeight: 600 }}>Privacy Policy</a> and <a href="#" style={{ color: "#15803D", fontWeight: 600 }}>Provider Code of Conduct</a>
            </span>
          </label>
        </>
      );
  ...
  return (
    <div style={{ width: "100%", maxWidth: 420 }}>
      <button onClick={onBack} style={{ background: "none", border: "none", cursor: "pointer", color: "#6B7280", fontSize: 13, fontWeight: 500, padding: 0, marginBottom: 14, display: "flex", alignItems: "center", gap: 6 }}>← Back</button>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
      ...
      <h1 style={{ fontSize: 20, fontWeight: 800, color: "#111827", margin: "6px 0 4px" }}>
          ...
          {(["login","signup"] as AuthPage[]).map((p) => (
            <button key={p} onClick={() => { if (p === "login") goLogin(); else goSignup(); }}
              style={{
                ...
                color: page === p ? "#15803D" : "#9CA3AF",
                border: "none",
                borderBottom: page === p ? "2.5px solid #15803D" : "2.5px solid transparent",
                transition: "all 0.2s",
              }}>
